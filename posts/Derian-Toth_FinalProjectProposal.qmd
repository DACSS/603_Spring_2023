...
This research exploration takes a close look at the gender wage gap across time. While the wage gap has decreased overall since 1980, women still make less than men. In 2022 women made on average, about 82 cents for every dollar men make (Aragao, 2023). This is shown in the line graph blelow In the line graph below (Female Salary Percent of Male Salary).
...
When comparing the top 10 occupation categories that are on average more dominated by women ("Occupation Categories Dominated by Women") with the list of occupation categories with the smallest wage gap ("Occupation Categories with the Smallest Wage Gap"), we can see some overlap. 
...
Further analysis is necessary to know if working in a female dominated occupation is a statistically significant factor that contributes to the wage gap.
...
A Tukey posthoc analysis was used to breakdown where the significant differences exited. 
...
To know if this relationship is significant, a regression analysis was run. Missing data were considered in this analysis. Data from the variable *wage_percent_of_male* were coded as NAs if the sample size of an occupation was too low. Therefore, these missing data was found to be "missing not at random" and we dropped the missing data in the regression analyses. However is it noted since this could bias the data as it does not include the entire data-set's set of occupations.  
