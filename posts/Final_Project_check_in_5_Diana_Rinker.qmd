...
We can also see consistent increase of this value over time, similar to
...
This table showing that google referrals are significant in either form - squared or logged. However, its coefficient is much higher when logged. Uniques variables's coefficient al;so changes significantly as I am modifying google variable. That migh indicate interaction between them.
First ,  I will log the rest of referral variables. 


```{r, echo=T}
model.4 <- lm(e.rate ~ log(Uniques)*I(google^2) +direct +other.web +social +bdc  +weekday , data = merged)
...
model.1,
library(stargazer)
stargazer( model.2,model.3, model.4,model.5, type = 'text')
...
Next,  I will check if the other referral varibles have ineraction with popularity (Uniques)
...
BIC
```
All six graphs demonstrate that residuals are 
