...
summary(model.11)
```
## 3.4. Visualizing interaction wbetween variables for the exit rate model: 

```{r, echo=T}
library(jtools)
library(interactions)
model.12 <- lm(e.rate ~ log(Uniques)+ log(google)*log(direct) +other.web +social +log(bdc)   , data = merged)
# summ(model.12)
stargazer( model.9,model.10,model.11, model.12, type = 'text')

model.11 <- lm(e.rate ~ log(Uniques)*log(google) +log(Uniques)* log(direct) +other.web +social +log(bdc)   , data = merged)
# summary(x.model <- lm(google~ direct+other.web+social+bdc+ll, data=merged))
interact_plot(model.11, pred =Uniques , modx =  google, plot.points = TRUE )
```


### Moderating impact of google referrals on relationship between popularity and engagement: 

```{r, echo=T}
# interact_plot(model.11, pred = google, modx = Uniques)

interact_plot(model.11, pred =Uniques , modx =  google, plot.points = TRUE )
```
At lower levels of Google referrals, increase of popularity (uniques) decreases engagement more rapidly, than at higher levels of google referrals.  I.e. at some level of Google referrals (~15-30K) poupularity impacts engagement not as strongly. 
...
# sum(is.na(merged$direct))
```
Similarly, at lower levels 

This graph demonstrates that with the lower values of direct referrals, exit rates are higher for the same amout of unique users. 
