...
2. Sometimes I feel as if I don’t belong in this class(`bel_2` and `bel_2_r`)
3. I am included in lots of activities in this class(`bel_3`)
4. I feel very different from most other students in this class (`bel_4` and `bel_4_r`)
5. I wish I were in a different class (`bel_5` and `bel_5_r`)
...
sum_bel_year <- Summarize(bel_sum ~ year,
                data=data,
                digits=3)
...
ggplot(sum_bel_year, aes(x = year,
                y = mean)) +
    geom_errorbar(aes(ymin = Q1,
                      ymax = Q3),
                   width=.2, size=0.7, position = (position_dodge(.2))) +
...
boxplot(bel_sum ~ year,
        data=data,
        ylab="Value",
        xlab= "Belonging")

# sum of bel, race, and gender
...
ggplot(sum_bel, aes(x = race,
    ...
    geom_errorbar(aes(ymin = Q1,
                      ymax = Q3),
                   width=.2, size=0.7, position = (position_dodge(.6))) +
    geom_point(shape=15, size=4, position = (position_dodge(.6))) +
    theme_bw() +
    theme(axis.title = element_text(face = "bold")) +
    ylab("Belonging") +
    xlab("Race") +
...
# sum of bel, race, and year

sum_bel_2 <- Summarize(bel_sum ~ race + year,
                data=data,
                digits=3)
...
boxplot(bel_sum ~ year + race,
        data=data,
        names=c("a.1", "a.2", "a.3", "a.4", "b.1", "b.2", "b.3", "b.4", "h.1", "h.2", "h.3", "h.4", "o.1", "o.2", "o.3", "o.4", "w.1", "w.2", "w.3", "w.4"),
        ylab="Value",
        xlab= "Belonging")
...
anova_race <- aov(bel_sum ~ race, data =data)

summary(anova_race)

ggplot(data, aes(x = race, y = bel_sum, color=race))+
    geom_boxplot()+
    ylab("Belonging")+xlab("Race")

# gender

anova_gender <- aov(bel_sum ~ gender, data =data)

summary(anova_gender)

ggplot(data, aes(x = gender, y = bel_sum, color=gender))+
    geom_boxplot()+
    ylab("Belonging")+xlab("Gender")

# year

anova_year <- aov(bel_sum ~ year, data =data)

summary(anova_year)

ggplot(data, aes(x = year, y = bel_sum, color= year, group = year))+
    geom_boxplot()+
    ylab("Belonging")+xlab("Year")

# year

anova <- aov(bel_sum ~ race + gender + year, data =data)
