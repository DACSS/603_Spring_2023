...
table(treecensus$borough)%>%
  barplot(xlab = "Borough", ylab = "Frequency")
...
ggplot(treecensus_clean, aes(x = borough, y = health)) +
    geom_boxplot(aes(color=borough))+
    ylab("Health")+ 
    xlab("Borough") + 
  scale_color_discrete(name = "Borough")
...
AIC(slrtree3, k=2)  
```
The best model for AIC is model 3. 
...
BIC(slrtree3)
```
The best model for BIC is model 3. 
...
```
The best model for PRESS is model 3.  
...
summary(treehealth_model)
healthtable <- coef(summary(treehealth_model))
p <- pnorm(abs(healthtable[, "t value"]), lower.tail = FALSE) * 2
healthtable <- cbind(healthtable, "p value" = p)
healthtable
treehealth_model2 <- polr(as.factor(health) ~ borough + steward, data = treecensus_clean, Hess=TRUE)
...
From this data, we can conclude that health is only significant within the Borough of Queens and with the confounder of steward.  
