...
```
Here we can see that the  points  form a curve instead of straight line for all three data sets, which means that our sample data are skewed.  This will confirm if we plot the Exit rat and author.sum:
...
Lets also look at distribution of exit rate per n.comments. Despite this distribution being  skewed, its shape is closer to normal: 
```{r, echo=T}

library(gridExtra)
plot1<- ggplot (merged, mapping =aes(y=log(`Exit rate`), x=n.comments))+
  geom_point()+
  geom_smooth(method="lm")
plot2<- ggplot(merged, aes(x = n.comments, y = log(`Exit rate`))) +
  geom_bar(stat = "identity")
grid.arrange(plot1, plot2, ncol=2)
```

Also., the weekday, another significant variable, is a categorical variable. 
...
We can see that the exit rate is lowest on Mondays
Lets plot number of omments and exit rates per dsy of the week. 
...
As we reviewd distributions of  independent variables  per  Exit rate, we saw that author's comments  are not normally distributed, which contributed to curve in QQ plot. 
...
```
 ...
 We cn conclude that the  heteroskedasticity of the data
