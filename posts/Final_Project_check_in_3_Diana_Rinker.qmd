...
```
Below is the summary of estimates and p-values  for three models, derived from the three different data sets: all data, 2021 and 2022. Comparing them will allow to identify whether the  
...
#three models with different variables 
model.1 <- lm(`Exit rate` ~ log(Uniques)+ author.sum  , data = merged)
model.2 <- lm(`Exit rate` ~ log(Uniques)+ author.sum  +n.comments , data = merged)
model.3 <- lm(`Exit rate` ~ log(Uniques)+ author.sum  +n.comments + pct.positive, data = merged)
model.4<- lm(`Exit rate` ~ log(Uniques)+ author.sum   +n.comments  +pct.positive +blocked.sum , data = merged)
model.5<- lm(`Exit rate` ~ log(Uniques)+ author.sum   +n.comments  +pct.positive +blocked.sum + post.weekday , data = merged)
model.6<- lm(`Exit rate` ~ log(Uniques)+ author.sum   +n.comments  +blocked.sum + post.weekday , data = merged)

library(stargazer)
stargazer(model.0, model.1, model.2, model.3, model.4, model.5, type = 'text')

```

This table shows that in addition to post popularity (log(Uniques)), which contributes the most to the engagement increasing exit rates,  there are other significant variables, such as 
- authors comments, our main independent variable of the research question. 
- number of comments, measurement of engagement of loyal readers. 
- blocked.sum
- post weekday 
 However, the coefficients for number of comments and number of blocked comments are negligibly small indicating  that their influence is realy, but tiny. Coefficients  for a weekday and author's comments are statistically significant and their estimates are , even though small, are bigger that the other variables. 
 
 ### Now I will use the same independent variables to calculate estimated coefficients based on  three to datasets. The result will allow me to see if difference between models produced based on different training data: 
...
```
QQ plots showing some deviation from  straight line on the edges, suggesting skewed distribution of residuals from all three models. 
...
```
Scale-location of compete dataset, combining 2021 and 2022 showing also some heteroscedasticity in distribution oif the data. Since 2021 and 2022 seem more heteroscedastistic,  this difference in the combined dataset might indicate significant differences betweek 2021 and 2022 data. 
...
```
Cooks distance demonstrates some outliers in each dataset.  We can review REsidualsvs Leverage plot to find out if these outliers impact our regression models: 
```{r, echo=T}
...
Since the diagnostic lots suggested non-linearity as the way to improve the model,  I will plot dependent variable against each Iv to see which of them might need to be logged or squared: 
...
```{r, echo=T}
library(gridExtra)
plot1<- ggplot (merged, mapping =aes(y=log(`Exit rate`), x=author.sum))+
  geom_point()+
  geom_smooth(method="lm")
plot2<- ggplot(merged, aes(x = as.factor(author.sum), y = log(`Exit rate`))) +
...
```{r, echo=T}
library(gridExtra)
plot1<- ggplot (merged, mapping =aes(y=`Exit rate`, x=n.comments))+
  geom_point()+
  geom_smooth(method="lm")

plot2<- ggplot(merged, aes(x = n.comments, y = `Exit rate`)) +
  geom_bar(stat = "identity")
grid.arrange(plot1, plot2, ncol=2)
```
Despite this distribution being  skewed, its shape is closer to normal.

### Exit rate ~ week day plot :
Weekday is another significant variable. It is also a categorical variable. 

```{r, echo=T}
plot1<- ggplot (merged, mapping =aes(y=`Exit rate`, x=post.weekday))+
  geom_point()+
  geom_smooth(method="lm")

plot2<- ggplot(merged, aes(x = post.weekday, y = `Exit rate`)) +
  geom_bar(stat = "identity")
grid.arrange(plot1, plot2, ncol=2)
```
The plot shows significant difference of exit rate on mondays, consistent with the coefficients of all three models. 

<!-- ```{r, echo=T} -->
<!--  ggplot (merged, mapping =aes(y=`Exit rate`, x=n.comments))+ -->
<!--   geom_point()+ -->
<!--   geom_smooth(method="lm")+facet_wrap(~post.weekday) -->

<!--  ggplot(merged, aes(x = n.comments, y = `Exit rate`)) + -->
<!--   geom_bar(stat = "identity") -->

<!-- ``` -->
<!-- It appears that Monday and Tuesday this relationship between number of comments nad exit rate is the most pronounced.  -->


As we reviewd distributions of  independent variables  per  Exit rate, we saw that author's comments  are not normally distributed, which might have contributed to curve in QQ plot. 
...
#Modifying data to log(auth.sum) 
 
```{r, echo=T}
model.6<- lm(`Exit rate` ~ log(Uniques)+ log(author.sum)   +n.comments  +blocked.sum + post.weekday , data = merged)
...
model.8 <-  lm(`Exit rate` ~ log(Uniques) +I(author.sum^2)  + I(n.comments^2)  +blocked.sum + post.weekday , data = merged)
library(stargazer)
...
# Plotting predicted against  data 
```{r, echo=T}
# colnames(merged)
model.all <- lm(`Exit rate` ~ log(Uniques)+ author.sum + pct.positive +blocked.sum+ post.weekday +n.comments, data = merged)
...
merged.pred1 <- merged%>%
mutate (predicted = predict(model.all))

plot.all<-ggplot (merged.pred1, mapping =aes(y=`Exit rate`, x=predicted))+
  geom_point()+
  geom_abline(slope = 1, intercept = 0)+
  labs(title="for alldata")

## 2021 test 
merged.pred2021 <- merged.2021%>%
      mutate (predicted.y = predict(model.all,newdata = merged.pred2021))


merged.2021 <- merged.2021 %>%
  mutate (predicted.y = predict(model.all))
dim(merged.2021)


plot.2021<-ggplot (merged.pred2021 , mapping =aes(y=`Exit rate`, x=predicted)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0)+
  labs(title="for 2021")

## 2022 test 
merged.pred2022 <- merged%>%
mutate (predicted = predict(model.all))

plot.2022<-ggplot (merged.2022, mapping =aes(y=`Exit rate`, x=predicted)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0)+
  labs(title="for 2022")


grid.arrange(plot.all, plot.2021,plot.2022,  ncol=3)
...
## 2021 test 
merged.pred2021 <- merged%>%
        mutate (predicted = predict(model.2021))
...
## 2022 test 
merged.pred2022 <- merged%>%
mutate (predicted = predict(model.2022))

plot.2022<-ggplot (merged.pred2022, mapping =aes(y=`Exit rate`, x=predicted)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0)+
  labs(title="for 2022")
