...
This is the most general metric, representing how many views
...
### Number of comments: absolute and  per views. 
...
```
The trend of comments is different. While the first four variables seemed to increase over time, this variable  decreases.  If we compare absolute values fo comments and ratio of comments per page views,  we see the decrease in both cases, while the ratio's decrease is more dramatic. 

It is important to note, that commenting on the platform  requires logging in from a user, which is an indicator of greater engagement of an individual user. 

It is also more likely that the users who logged and commented in are likely to return to the post (=add views or visits while not changing "uniques" count). If that assumption holds, an increase of first for variables (vies, visits and uniques) implies with decrease of comments, which is so far supported by the data. 
...
```
Number of reactions appears to decrease over time. To like/dislike the comment, user doesn't have to log in. 



##Page views : relationship between popularity and engagement. 
...
### Adding weekdays to the model - BROKEN HERE 
...
This graph shows, that majority of posts have no author's comments. However, if we look at the boxplots' upper ranges, we can see a trend of having overall less author's comments with time.
...
# summary(lm(visits.ratio ~ comments.ratio+  reactions.ratio  , data = merged))
# summary(lm(visits.ratio ~ n.comments+  reactions.ratio  , data = merged))
# summary(lm(visits.ratio ~ n.comments+  reactions.ratio  , data = merged))
...
summary(lm(comments.ratio ~  author.sum  +pct.positive+ blocked.sum , data = merged))
