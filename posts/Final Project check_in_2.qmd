...
```
Overal trend of comments is different. While the first four variables seemed to increase over time, this variable overall decreases.  If we compare absolute values fo comments and ration of comments per page views,  we see the decrease in both cases, while the ratio's decrease is more dramatic. 
...
str(merged)
  merged <- merged %>%
     mutate(likes.ratio = post.total.likes / `Page views`)
  
# str(merged)
ggplot(merged, mapping = aes(x=year_month , y=likes.ratio, fill=year_month ))+
  geom_boxplot() +
...
```
Distributions of reactions appears to decrease over time. To like/dislike the comment, user doesn't have to log in. 
...
```
Thes appears as downward trend, however,nees to be tested for significance. 
...
ggplot(merged, mapping=aes(x=post.date))+
  ...
  labs(title = "referral sources per post ", y = "Number of referrals" , x="Post")+ 
     theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
This graph clearly demonstrates increase of "Search + amp referral visits" after December 2021, while other referral sources maintain the same level over time. This increase matches changes observed in Page Views, visits and uniques and might be associated with the trend of less engaged users, observed in  uvisits and uniques ratios. Overall decrease of author's comments might also correspond to this. 


# Analysis

First, I will use "page views" as dependent variable and regress it against all other variables representing engagement: 
...
# colnames(merged)
 ...
 merged <- merged %>%
     mutate(adj.page.voiews =`Page views` / 10000)

summary(lm(adj.page.voiews ~ `Exit rate`+  uniques.ratio +visits.ratio  +comments.ratio+  likes.ratio , data = merged))
summary(lm(adj.page.voiews ~ `Exit rate`+  uniques.ratio +visits.ratio  +comments.ratio+  likes.ratio +  pct.positive+ author.sum  + blocked.sum , data = merged))
```
The model is showing  significant correlation between all of engagement metrics we have identified. Some of them having negative correlation with the page view, they are still significant indicator of POPULARITY.
...
Therefore # 1 and #3 are not necessarily negatively correlated. In our data we can see that scenarios 1 and 3 are increasing  as overall page views increase. 

Correlation with comments rationis is significant and negative. We can conclude, that while more popularposts are much less commented.

Likes ratio shows significant  negative correlation with total views, indicating that users are less likely to react to comments of posts with larger amount of views. 
```{r, echo=T}

summary(lm(adj.page.voiews ~ likes.ratio + n.comments+ author.sum  + blocked.sum , data = merged))
