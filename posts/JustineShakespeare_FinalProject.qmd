...
[^21]: International Monetary Fund. GDP per capita, current prices. https://www.imf.org/external/datamapper/NGDPDPC\@WEO/OEMDC/ADVEC/WEOWORLD
...
ggplot(world_data, aes(x = long, y = lat, group = group, fill = Maternal_Mortality_Ratio_2017)) +
  geom_polygon() +
  scale_fill_gradient(low = "#E2D0CA", high = "#D84C34") +
...
Despite the lower R\^2 values, the model without the interaction term and without the `Expend_Health_pct_GDP` variable (MMR_3) has the lowest AIC and BIC numbers, with 391.8238 and 410.0844 respectively. The model without the interaction term (MMR_1) is slightly weaker as the AIC and BIC are slightly larger, with the AIC 392.101 and BIC 413.405. The model with the interaction term (MMR_2) is the weakest, as it has the largest AIC and BIC: 393.7846 and 418.132, respectively.

Finally, let's compare the PRESS statistic of all three models.
...
**logMMR2017 = 12.60 + 0.3347logGenderExclusion + 0.0128Democracy - 0.0246SkilledBirthAttendant - 0.7031logGDP2017**
...
The first indirect effect model show that the `logGenderExclusion` variable is statistically significant without any covariates, but is no longer significant when the control variables are included.

The second indirect effect model will show the relationship between the mediator and the dependent variable while controlling for the explanatory variable.
...
In both of these second indirect effect models the explanatory variable is still statistically significant with the mediator present in the model. The model with the `logGenderExclusion` variable and all of the covariates is stronger than the models without covariates when comparing the R\^2 values.
