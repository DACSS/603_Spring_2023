...
long_data <- x_y_controls_df %>%
  select(year, total_food_per_capita, ibis_grcy_revenue_usd_m_per_capita_state, gt_abs_sustain_per_capita, gt_abs_cc_per_capita) %>%
  gather(key = "variable", value = "value", -year)

# Create box and whisker plot for multiple variables
ggplot(data = long_data, aes(x = factor(year), y = value, fill = variable)) +
  geom_boxplot() +
  ggtitle("Box and Whisker Plot for Multiple Variables by Year") +
  xlab("Year") +
  ylab("Value") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14)) +
