...
I fit several different models.

Model 1:
`log(total_food_per_capita) ~ log(gt_abs_sustain_per_capita) + log(gt_abs_cc_per_capita) + interaction_term + year + ibis_grcy_revenue_usd_m_per_capita_state + population, data=x_y_controls_df, weights=x_y_controls_df$weights`
...
1.  **`log(gt_abs_sustain_per_capita)`**: A 1% increase in
    ...
    decreases. This finding is statistically significant.
...
2.  **`log(gt_abs_cc_per_capita)`**: A 1% increase in log(climate change
    ...
    positive relationship is counterintutive. However, the results are
    not statistically significant.

3.  **`interaction_term`**: The interaction term captures the combined
    effect of climate change and sustainabilty search volume on total
    food per capita. A 1-unit increase in the interaction term is
    associated with a 0.001-unit increase in the log total food waste
    per capita, holding all other variables constant

Model 2:
`log(total_food_per_capita) ~ log(gt_abs_sustain_per_capita) + log(gt_abs_cc_per_capita) + year + ibis_grcy_revenue_usd_m_per_capita_state + population, weights=x_y_controls_df$weights`

-   Uses
    `log(gt_abs_sustain_per_capita) + log(gt_abs_cc_per_capita) +ibis_grcy_revenue_usd_m_per_capita_state`
    as explanatory variables and drops the interaction term
...
fit_no_interaction <- lm(log(total_food_per_capita) ~ log(gt_abs_sustain_per_capita) + log(gt_abs_cc_per_capita) + year + ibis_grcy_revenue_usd_m_per_capita_state + population, data=x_y_controls_df, weights=x_y_controls_df$weights)

fit_no_weights <- lm(log(total_food_per_capita) ~ log(gt_abs_sustain_per_capita) + log(gt_abs_cc_per_capita) + interaction_term + year + ibis_grcy_revenue_usd_m_per_capita_state + population, data=x_y_controls_df)
```
...
stargazer(fit_inter_pop, fit_no_int_no_cc,type = "text",
          ...
          align = TRUE,
          column.labels = c("M1:Interact/Pop", "M2:NoInteract"),
