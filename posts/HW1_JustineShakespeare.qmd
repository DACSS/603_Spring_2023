...
We can use the mutate() and case_when() functions and piping to create an ordinal variable that captures different age groups:
...
With this new variable we can examine the relationship between lung capacity, age, and smoking status. The following tables show the mean lung capacity for study subjects arranged by age group, first for those who reported not smoking and second for those who did report smoking:
